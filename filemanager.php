...
function sanitizePath($path) {
    ...
    if ($realPath === false) $realPath = realpath(dirname($path));
    $rootReal = realpath(ROOT_DIR);
    if ($realPath === false || strpos($realPath, $rootReal) !== 0) return $rootReal;
    return $realPath;
}
            ...
            case 'get_tree':
                function buildTree($d) {
                    ...
                    foreach ($items as $i) {
                        if ($i==='.'||$i==='..') continue;
                        $f = $d.'/'.$i;
                        if (is_dir($f)) $t[] = array('name'=>$i,'path'=>$f,'children'=>buildTree($f));
                    }
                    usort($t, function($a,$b){ return strcasecmp($a['name'],$b['name']); });
                    return $t;
                }
                $root = realpath(ROOT_DIR);
                $res = array('success'=>true,'tree'=>array(array('name'=>basename($root),'path'=>$root,'children'=>buildTree($root))));
                break;
        }
...
$rootReal = realpath(ROOT_DIR);
$breadcrumbs = array();
$relativePath = str_replace($rootReal, '', $currentDir);
$pathParts = array_filter(explode('/', $relativePath));
$breadcrumbPath = $rootReal;
$breadcrumbs[] = array('name'=>'Root','path'=>$rootReal);
