        ...
        switch ($_POST['action']) {
            ...
            case 'get_folders':
                ...
                if ($items !== false) {
                    foreach ($items as $i) {
                        if ($i==='.'||$i==='..') continue;
                        $f = $path . DIRECTORY_SEPARATOR . $i;
                        if (is_dir($f)) {
                            $hasChildren = false;
                            $subItems = @scandir($f);
                            if ($subItems !== false) {
                                foreach ($subItems as $sub) {
                                    if ($sub !== '.' && $sub !== '..' && is_dir($f . DIRECTORY_SEPARATOR . $sub)) {
                                        $hasChildren = true;
                                        break;
                                    }
                                }
                            }
                            $folders[] = array('name'=>$i,'path'=>$f,'hasChildren'=>$hasChildren);
                        }
                    }
                    usort($folders, function($a,$b){ return strcasecmp($a['name'],$b['name']); });
                }
                $res = array('success'=>true,'folders'=>$folders);
                break;
        }
...
<div class="toolbar">
<button class="toolbar-btn" onclick="showHome()"><i class="fas fa-home"></i> Home</button>
...
<div class="home-subtitle">Quick access to your pinned folders and full directory tree</div>
</div>
<div class="pinned-section">
<div class="section-title">⭐ Pinned Folders</div>
